...
function countVowels(str) {
  ...
  const filteredChar = arrStr.filter(
    (el) =>
      el === 'a' ||
      el === 'e' ||
      el === 'i' ||
...
function isPalindrome(str) {
  const strWord = str
    ...
    .join('');
  const reversedWord = strWord.split('').reverse().join('');
  return strWord === reversedWord;
}
...
function findLongestWord(sentence) {
  ...
  const sortedWord = senArr.sort((a, b) => b.length - a.length);
  return sortedWord[0];
}
...
function reverseWords(str) {
  ...
  const reversedWord = wordsArr.map((el) => el.split('').reverse().join(''));
...
function invertCase(str) {
  const wordsArr = str.split(' ');
  const reversedCase = wordsArr.map((el) =>
    el
      .split('')
      .map((letter) => {
...
function extractNameFromTemplate(value) {
  ...
  return value.slice(7, strLength);
}
...
function unbracketTag(str) {
  const removeTag = str.split('').filter((el) => el !== '<' && el !== '>');
  return removeTag.join('');
}
